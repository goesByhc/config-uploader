...
BAST_CLASSPATH=$script_path/excel2plist/lib
...
CLASSPATH=$CLASSPATH:$script_path/excel2plist/bin

binpath=$script_path/excel2plist/bin/
if [[ ! -d $binpath ]]; then
  mkdir $binpath
fi
...
javac -Xlint:unchecked -classpath $CLASSPATH -d $binpath -sourcepath $script_path/excel2plist/src/ $script_path/excel2plist/src/com/inke/pb/*.java
if [[ $? -ne 0 ]]; then
    exit 1
fi
...
javac -Xlint:unchecked -classpath $CLASSPATH -d $binpath -sourcepath $script_path/excel2plist/src $script_path/excel2plist/src/com/inke/conf/*.java
if [[ $? -ne 0 ]]; then
    exit 1
fi
